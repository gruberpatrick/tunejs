...
module.exports = {
	...
	// @param function : the callback Function for the Load Process
	//
	loadFilesInFolders: function(fProcess){
	...
	//------------------------------------------------------------------------------
	// Load Information from Module (Bitrate, etc.)
	//
	// @param integer : preventing the async function from opening too many files
	// @param function : called when finished
	//
	songInfoCheck: function(lCounter, fFinished){

		var aSongs = Object.keys(DB.oDB.aSongs);
		var oThat = this;

		if(typeof aSongs[lCounter] == "undefined" && typeof fFinished != "undefined"){
			fFinished(--lCounter);
			return;
		}else if(DB.oDB.aSongs[aSongs[lCounter]].lBitrate > 0){
			setTimeout(function(){ oThat.songInfoCheck(++lCounter, fFinished); }, 0);
			return;
		}

		Taglib.read(aSongs[lCounter], function(sErr, oTags, oProp){
			if(typeof oTags.title != "undefined" && typeof oTags.artist != "undefined" && typeof oTags.album != "undefined"){
				DB.oDB.aSongs[aSongs[lCounter]].sTitle = oTags.title || "";
				DB.oDB.aSongs[aSongs[lCounter]].sArtist = oTags.artist || "";
				DB.oDB.aSongs[aSongs[lCounter]].sAlbum = oTags.album || "";
				DB.oDB.aSongs[aSongs[lCounter]].lYear = oTags.year || 0;
				DB.oDB.aSongs[aSongs[lCounter]].lTrack = oTags.track || 0;
				DB.oDB.aSongs[aSongs[lCounter]].sComment = oTags.comment || "";
				DB.oDB.aSongs[aSongs[lCounter]].lLength = oProp.length || 0;
				DB.oDB.aSongs[aSongs[lCounter]].lBitrate = oProp.bitrate || 0;
				DB.oDB.aSongs[aSongs[lCounter]].lSampleRate = oProp.sampleRate || 0;
				DB.oDB.aSongs[aSongs[lCounter]].lChannels = oProp.channels || 2;
			}
			setTimeout(function(){ oThat.songInfoCheck(++lCounter, fFinished); }, 0);
		});
	...
	//------------------------------------------------------------------------------
	// Search for a Song by String
	//
	// @param string : value to search
	//
	// @return array : result Songs
	//
	searchSong: function(sSearch){
		var aRes = [];
		var aSearch = sSearch.split(" ");
		for(var sSong in this.oDB.aSongs){
			var found = true;
			for(var i in aSearch){
				var sSearch = aSearch[i].trim().toLowerCase();
				if((typeof this.oDB.aSongs[sSong].sTitle == "undefined") || (this.oDB.aSongs[sSong].sTitle.toLowerCase().indexOf(sSearch) < 0 && this.oDB.aSongs[sSong].sArtist.toLowerCase().indexOf(sSearch) < 0 && this.oDB.aSongs[sSong].sAlbum.toLowerCase().indexOf(sSearch) < 0)){
					found = false;
					break;
				}
			}
			if(found)
				aRes.push(this.oDB.aSongs[sSong]);
		}
		return aRes;
	}
